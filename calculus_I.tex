...
\begin{enumerate}[label=\alph*)]
  ...
  \textbf{Nota:} En esta proposición se pide que $s\leq j\leq t$, sin embargo, resulta interesante observar los casos sin esta restricción:
  \begin{enumerate}[label=\roman*)]
  \item Si $s>j>t$,
  \begin{align*}
  ...
  \item \[\sum_{n=a}^{b}f(n) = \sum_{n=0}^{b-a} f(b-n)\] (la suma del primer termino hasta el ultimo es igual a la suma del ultimo hasta el primero)

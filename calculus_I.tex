...
\textbf{Nota:} El lector debería verificar que ambas definiciones son equivalentes. \textit{Hint:} La suma del primer termino hasta el ultimo es igual a la suma del ultimo hasta el primero.
...
Dado que contamos con una definición que puede ser planteada de dos maneras, podemos utilizar cualquiera (de las dos) a conveniencia, pues siempre se obtiene el mismo resultado; por ejemplo:
\begin{center}
  \begin{minipage}[c]{.5\linewidth}
    \begin{align*}
...
\begin{enumerate}[label=\alph*)]
  ...
  \bfit{Corolario:} Si $c\in \R$ y $n\in \N$, \[\sum_{i=1}^n c = nc\]
  ...
  \bfit{Corolario:} \[\sum_{n=a}^b f(m+n) = \sum_{n=a+c}^{b+c} f(m+n-c)\]
  \begin{proof}\leavevmode
    \begin{enumerate}[label=\Roman*)]
      \item Si $b=a$, 
      \begin{align*}
        \sum_{n=a}^b f(m+n) &= \sum_{n=a}^a f(m+n)\\
        &= f(m+a)\\
        &= \sum_{n=a+c}^{a+c} f(m+n-c)
      \end{align*}
      ...
      \begin{enumerate}[label=\roman*)]
        ...
        \begin{align*}
          \sum_{n=a}^b f(m+n) &= \sum_{n=a}^{a+1} f(m+n)\\
          &= f(m+a+1) + \sum_{n=a}^{a} f(m+n)\\
          &= f(m+a+1) + f(m+a)\\
          &= f(m+a+1) + \sum_{n=a+c}^{a+c} f(m+n-c)\\
          &= \sum_{n=a+c}^{a+c+1} f(m+n-c)\\
          &= \sum_{n=a+c}^{b+c} f(m+n-c)
        \end{align*}

        \item Supongamos que se verifica para $b=a+k$, es decir, suponemos que
        \begin{align*}
          \sum_{n=a}^{a+k} f(m+n) &= \sum_{n=a+c}^{a+k+c} f(m+n-c)
        \end{align*}

        \item Si $b=a+k+1$,
        \begin{align*}
          \sum_{n=a}^{a+k+1} f(m+n) &= f(m+a+k+1) + \sum_{n=a}^{a+k} f(m+n)\\
          &= f(m+a+k+1) + \sum_{n=a+c}^{a+k+c} f(m+n-c) && \text{Hip. Inducción}\\
          &= \sum_{n=a+c}^{a+k+1+c} f(m+n-c)
        \end{align*}
      \end{enumerate}
  ...
  \item Si $s=j>t$,
  \begin{align*}
    \sum_{n=s}^{j} f(n) + \sum_{n=j+1}^{t} &= f(n) + 0 = f(n) \neq 0 = \sum_{n=s}^{t} f(n)
  \end{align*}
  Por lo que descartamos este caso.
  \end{enumerate}

  El lector notará que para partir la suma en el caso $s=j>t$, debe cumplirse que $f(n)=0$, pero esto dependerá de cada función y de los íncides $s=j$, por lo que en general, $f(n)\neq 0$, por ejemplo para cualquier sumatoria $\sum_{n=a}^{b} c$, donde $c\neq 0$.
  ...
  \begin{proof}\leavevmode
    ...
    \begin{enumerate}[label=\roman*)]
      ...
      \item Supongamos que se verifica para $b=a+k$, con $k\in \N$, es decir, suponemos que
      \begin{align*}
        \sum_{n=a}^{a+k}f(n) &= \sum_{n=0}^{(a+k)-a} f(a+k-n)\\
        &= \sum_{n=0}^{k} f(a+k-n)
      \end{align*}
      ...
      \begin{align*}
        \sum_{n=a}^{b} f(n) &= \sum_{n=a}^{a+k+1} f(n)\\
        &= f(a+k+1) + \sum_{n=a}^{a+k} f(n)\\
        &= f(a+k+1) + \sum_{n=0}^{k} f(a+k-n) && \text{Hip. Inducción}\\
        &= \sum_{n=-1}^{k} f(a+k-n)\\
        &= \sum_{n=0}^{k+1} f(a+k-n-1) && \text{Cambio de índice}\\
      \end{align*}
    \end{enumerate}
  ...
  \item \[\sum_{n=a}^b f(n) = \sum_{n=0}^b f(n) - \sum_{n=0}^{a-1} f(n)\]
  \begin{proof}\leavevmode
    Pendiente
  \end{proof}

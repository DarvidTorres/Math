...
\begin{enumerate}[label=\alph*)]
  ...
  \begin{enumerate}[label=\Roman*)]
    \item Notemos que si $c=0$, la proposición es \textit{tautológica}; por lo que suponemos en todo caso que $c\neq 0$.
    ...
    Si $c\in \Z\setminus \set{0}$, $\ell \in \R$, entonces \[\sum_{n=a}^b f(\ell+n) = \sum_{n=a+c}^{b+c} f(\ell+n-c) \qquad \text{(Cambio de índice 1)}\]
    
  \end{enumerate}

  \item Sea $c\in \Z\setminus \set{0}$, $m\in \R$, \[\sum_{n=a}^b f(m-n) = \sum_{n=a+c}^{b+c} f\bigl(m-(n-c)\bigr) \qquad \text{(Cambio de índice 2)}\]
  \begin{proof}\leavevmode
    \begin{enumerate}[label=\roman*)]
    \item Si $a>b$,
    \begin{align*}
  ...
  \begin{proof}\leavevmode
    ...
    \begin{enumerate}[label=\roman*)]
      ...
      \begin{align*}
        ...
        &= \sum_{n=-1+(1)}^{k+(1)} f\bigl(a+k-(n-1)\bigr) && \text{Cambio de índice 2}\\

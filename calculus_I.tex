...
\textbf{Definición:} Sea $|b-a|\in \Z$ y $f: \R\to \R$, definimos a la sumatoria como sigue:
...
\begin{enumerate}
  \item \begin{align*}
    ...
    &= 4 + 9 + (4)^2 + \sum_{i=5}^{5} i^2 && \text{(*)}\\
    &= 4 + 9 + 16 + (5)^2 + \sum_{i=6}^{5} i^2\\
    &= 4 + 9 + 16 + 25 + 0\\
    &= 54
  \end{align*}
  ...
  \item \begin{align*}
    ...
    &= -6 + 2(-2) + \sum_{m=-1}^{-1} 2m  && \text{(**)}\\
    &= -6 + -4 + 2(-1) + \sum_{m=0}^{-1} 2m\\
    &= -6 + -4 + -2 + 0\\
    &= -12
  \end{align*}
  ...
  \item \begin{align*}
    ...
    &= 1 + 2 + \sum_{n=3}^{3} 2^n  && \text{(***)}\\
    &= 1 + 2 + 2^3 + \sum_{n=4}^{3} 2^n\\
    &= 1 + 2 + 8 + 0\\
    &= 11
  \end{align*}
...
\end{enumerate}
  ...
  El lector notará que, en el caso en que los límites inferior y superior son iguales (*, **, y ***), la imagen de la sumatoria es el elemento típico \textit{evaluado} en el índice, es decir,

  \textbf{Observación:} Si $a=b$, entonces
  \begin{align*}
    \sum_{n=a}^{b} f(n) &= f(n) && \text{(Límites iguales)}
  \end{align*}

  \begin{proof}\leavevmode
    Sea $m\in \Z$,
  \begin{align*}
    \sum_{n=m}^{m} f(n) &= f(m=n) + \sum_{n=m+1}^{m} f(n) && \text{Definición}\\
  ...
  \[
    ...
    \textbf{Observación:} Sea $a,b\in \R$ arbitrarios pero fijos tales que $a\leq b$, entonces
    \begin{align*}
      \sum_{n=a}^{b} f(n) = f(b) + \sum_{n=a}^{b-1} f(n) && \text{(Suma inversa)}
    \end{align*}
    ...
    \begin{proof}\leavevmode
      \begin{enumerate}[label=\Roman*)]
        ...
        \item Para el caso $a<b$, procedemos por inducción sobre $b-a$.
        \begin{enumerate}[label=\roman*)]
          \item Se verifica para $b=a+1$,
          \begin{align*}
            \sum_{n=a}^{b=a+1} f(n) &= f(a) + \sum_{n=a+1}^{a+1} f(n) && \text{Definición}\\
          ...
          \begin{align*}
            \sum_{n=a}^{a+k} f(n) &= f(b) + \sum_{n=a}^{b-1} f(n)
          \end{align*}

          \item Notemos que
          \begin{align*}
            \sum_{n=a}^{b=a+k+1} f(n) &= f(a) + \sum_{n=a+1}^{a+k+1} f(n) && \text{Definición}\\
            &= f(a) + f(b) + \sum_{n=a+1}^{a+k+1} f(n)\\
            &= f(a+k+1) + f(b) + \sum_{n=a}^{b-1} f(n)\\
...
\textbf{Nota:} El lector debería verificar que ambas definiciones son equivalentes. \textit{Hint:} 
...
\begin{enumerate}[label=\alph*)]
  ...
  \bfit{Nota:} En esta proposición se restringe que $a\leq b$, pues si $a>b$, se tiene que $\sum_{n=a}^{b} c = 0 \neq (b-a+1)c$; únicamente en el caso que $c=0$, se cumpliría la igualdad con $a>b$.
...
Se observa que, estrictamente, se abusa de la notación; pues si tenemos enteros $a,b\in \Z$ tales que $a<b$, hemos definido $\sum_{n=a}^{b} f(n) = f(a) + \sum_{n=a+1}^{b}f(n)$; el lector notará que del lado izquierdo de esta igualdad $n=a$ pero del lado derecho $n=a+1$, es decir, se emplea el mismo símbolo $n$ para números enteros distintos, lo que resulta útil en este caso ya que la definición de sumatoria está dada \textit{recursivamente}, sin embargo, de manera minuciosa (y pedante), se podría reescribir, para $b>a$, como $\sum_{n=a}^{b}f(n) = f(a) + \sum_{n+1=a+1}^{b}f(n+1)$, pero en cada iteración tendríamos que cambiar el índice (inferior) y, con ello, la variable a evaluar, lo que llegaría a ser laborioso, especialmente si se requiere de expresar múltiples iteraciones; por ello, se acude al abuso mencionado.

\textbf{Extensión}

Usualmente, el alcance de una suma se extiende al primer símbolo de suma o resta que no está entre paréntesis o es parte de algún término más amplio (por ejemplo, en el numerador de una fracción), de manera que:

...
\newcounter{counter}
\newcommand{\counter}{\stepcounter{counter}\arabic{counter}}
%new list type for exercises
\usepackage{enumitem}
\newlist{exercises}{enumerate}{1}
\setlist[exercises]{%
label=LE\arabic{counter}.\arabic*, wide = 0pt, labelwidth = 3em, labelsep = 0.3333em, leftmargin = \dimexpr\labelwidth + \labelsep\relax}

%counter for aximos
\newcounter{axioms}
\newcommand{\axiom}{\stepcounter{axioms}\arabic{axioms}}
...
\begin{enumerate}[label=Axc.\arabic*, wide = 0pt, labelwidth = 3em, labelsep = 0.3333em, leftmargin = \dimexpr\labelwidth + \labelsep\relax ]%
  ...
  \item Elemento neutro para la suma. Existe un número real llamado elemento neutro para la suma o cero, denotado por $0$, el cual satisface la siguiente condición: $ m+0=m,\forall m \in \R $.
...
\section*{Lista de Ejercicios \counter}
...
Existe un subconjunto del conjunto de los números reales llamado conjunto de los números reales positivos, denotado con el símbolo $\R^+$. A los elementos de este conjunto los llamaremos números reales positivos.\\
El conjunto $\R^+$ satisface los siguientes axiomas:
...
\begin{enumerate}[label=\alph*)]
    ...
    \item Por definición $b-a \in \R^+$, y por (b) de LE3 $c \in \R^+$. Luego, por O2 se verifica que $c(b-a) \in \R^+$. Por A9 sigue que $c(b-a)=cb-ca$ y por A5 tenemos que $cb-ca=bc-ac$. De este modo, $bc-ac \in \R^+$, es decir, $ac<bc$.
    \qed
    ...
    \begin{align*}
        ...
        &= c-a && \text{Por A1}\\
    \end{align*}
    Entonces $c-a \in \R^+$, es decir, $a<c$.
    \qed
...
\begin{enumerate}[label=\alph*)]
    ...
    \item Supongamos que para toda $U_\varepsilon(a)$ y $V_\varepsilon(b)$ se cumple que $U_\varepsilon(a) \cap V_\varepsilon(b) \neq \emptyset$. Entonces, existe $x$ tal que $x\in U_\varepsilon(a)$ y $x\in V_\varepsilon(b)$. Como en ambas vecindades tenemos $\varepsilon>0$ arbitraria, por (a) de LE5, sigue que $x=a$ y $x=b$, pero esto contradice el supuesto de que $a\neq b$. Por tanto, deben existir $U_\varepsilon(a)$ y $V_\varepsilon(b)$ tales que $U\cap V =\emptyset$. \qed
    \end{enumerate}
...
\begin{enumerate}[label=\arabic*.]\setcounter{enumi}{2}
    ...
    \item \begin{enumerate}[label=\roman*)]
        \item Sea $M$ una cota superior de $S$ tal que $\forall \varepsilon>0, \exists s_{\varepsilon}$ tal que $M-\varepsilon<s_{\varepsilon}$. Si $M$ no es el supremo de $S$, tendríamos que $\exists V$ tal que $s_@a \leq V < M$. Elegimos $\varepsilon = M-V$, con lo que $V<s_{\varepsilon}$, lo que contradice nuestra hipótesis. Por tanto, $M$ es el supremo de $S$.
        \item Sea $M$ el supremo de $S$ y $\varepsilon>0$. Como $M<M+\varepsilon$, entonces $M-\varepsilon$ no es una cota superior de $S$, por lo que $\exists s_\varepsilon$ tal que $s_\varepsilon>M-\varepsilon$.
        \end{enumerate}
        \qed
\end{enumerate}
...
\textbf{Definición:} Decimos que una secuencia $X$ converge a $\ell \in \R$, o que la secuencia es convergente, si $\exists n_0\in \N$ tal que los términos $x_n$ con $n\geq n_0$ satisfacen que $|x_n-\ell|<\varepsilon$, $\forall \varepsilon>0$.
...
\begin{enumerate}[label=\alph*),font=\bfseries]
    \item Sea $X$ una secuencia convergente. Por definición, $\exists n_0 \in \N$ tal que los términos $x_n$ con $n\geq n_0$ satisfacen que $|x_n-\ell|<\varepsilon, \forall \varepsilon>0$. Sea $\varepsilon_0>0$ arbitrario pero fijo, entonces \begin{align*}
        |x_n - \ell| &< \varepsilon_0 \\
        |x_n - \ell| + |\ell| &< \varepsilon_0 + |\ell|
    \end{align*}\\
    Luego, notemos que $|x_n| = |x_n-\ell+\ell|$, y por la desigualdad del triángulo, \[|x_n-\ell+\ell| \leq |x_n-\ell| + |\ell|\]\\ Por transitividad, $|x_n|< \varepsilon_0 + |\ell|$, lo que implica que $\{x_n\}$ con $n\geq n_0$ está cotado superiormente.
    ...
    Finalmente, el conjunto $\{x_{n<n_0}\} \cup \{\varepsilon_0 + |\ell|\}$ está acotado superiormente, por lo que $X$ está acotada. \qed
\end{enumerate}

...
\begin{itemize}
 ...
 \item $(-\infty, a) \defined \set{x\in \R|x<a}$.
 \item $(-\infty, a] \defined \set{x\in \R|x\leq a}$.
\end{itemize}
...
\begin{enumerate}[label=\roman*)]
  \item Los conjuntos $\R^+$ y $\R^-$ son intervalos de la forma $(0, \infty)$ y $(-\infty, 0)$.
  \item Para cualquier $a\in \R$ se tiene que $(-\infty, a]\union (a, \infty) = \R$. A partir de este hecho introducimos la notación $(-\infty, \infty) \defined \R$.
\end{enumerate}
...
\begin{enumerate}[label=\alph*)]
  ...
  \begin{proof}\leavevmode
    \begin{enumerate}[label=\roman*)]
      \item Como $\max(A) \in A$, entonces $\max(A)\in B$. Por definición, $\max(B)\geq b, \forall b\in B$. En particular, $\max(A)\leq \max(B)$.
      \item Como $\min(A)\in A$, entonces $\min(A)\in B$. Por definición, $\max(B)\leq b,\forall b\in B$. En particular, $\min(B)\leq \min(A)$. \qedhere
    \end{enumerate}
  \end{proof}
  
  \item Sea $A\subseteq \R$ y $-A\defined \set{-a| a\in A}$. Se verifica que $\max(A)=-\min(-A)$.
  \begin{proof}\leavevmode
    Notemos que $\min(-A)\leq -a, \forall -a\in A$. De esta desigualdad sigue que $a\leq -\min(A)$, y como esta desigualdad se verifica para cualquier $a\in A$, entonces $-\min(A)$ es una cota superior de $A$. Finalmente, $\min(-A)\in -A$, y por definición $-\min(-A)\in A$, es decir, $-\min(-A)=\max(A)$.
  \end{proof}
  
  \item Sean $a,b\in \R$. Se verifica que $\max\set{a, b} = \frac{a+b+|a-b|}{2}$ y $\min\set{a, b} = \frac{a+b-|a-b|}{2}$.
  \begin{proof}\leavevmode
    \begin{enumerate}[label=\roman*)]
      \item Si $b\leq a$, entonces $\max\set{a, b}=a$ y $\min\set{a, b}=b$. También, $a-b\geq 0$, por lo que $|a-b|=a-b$, de donde \begin{align*}
      ...
      \begin{align*}
        ...
        &= \min\set{a, b}
      \end{align*}
      \item Si $a< b$, entonces $\max\set{a, b}=b$ y $\min\set{a, b}=a$. También, $b-a> 0$, por lo que $-(b-a)=a-b< 0$, de donde $|a-b|=-(a-b)=b-a$. Luego,
      \begin{align*}
        \frac{a+b+|a-b|}{2} &= \frac{a+b+b-a}{2}\\
        &= \frac{2b}{2}\\
        &= b\\
        &= \max\set{a, b}
      \end{align*}

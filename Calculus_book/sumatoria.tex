...
\textbf{Definición:} Sea $I, A\subseteq \R$ y $f$ una función biyectiva dada por
\begin{align*}
  f : I &\to A\\
  i &\mapsto a
\end{align*}

definimos la notación índice como \[a_i \defined f(i)\]

\begin{itemize}
  \item Decimos que $A$ es un conjunto indexado.
  \item Decimos que $I$ es el índice de $A$.
\end{itemize}

\textbf{Definición:} Sea $A\propersubset \R$ y $M=\set{m\in \N|1\leq m\leq
n}$. Decimos que $A$ es un conjunto finito (indexado) si existe una
biyección $f: M \to A$, para algún $n \in \N$.
\begin{itemize}
...
\textbf{Observación:} Amenos que $A$ sea un conjunto \textit{unitario}, la función $f: M \to A$ no es única. El lector debería verificar este hecho.

Usualmente la sumatoria se presenta con la notación índice para conjuntos finitos indexados. Por ejemplo, \[\sum_{i=m}^{n} a_i\]

en la cual se tiene una función biyectiva
\begin{align*}
f : M &\to A\\
i &\mapsto a
\end{align*}
donde $M$ es el índice de $A$. La notación índice se define como:

\[a_i \defined (i, a)\in f\]

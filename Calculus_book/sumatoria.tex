...
\[ \binom{n}{k}\defined
  ...
  0 &\text{si} \quad k>n \ \text{, o} \ k<0
  \end{cases}
  \]

Otras notaciones para $\binom{n}{k}$ son $C(n, k), {}_nC_k, {}^nC_k, C_{n,k}$.

\textbf{Observación:} \[\binom{n}{0} = \frac{n!}{0!(n-0)!} = \frac{n!}{n!} = 1\]
  ...
  \begin{proof}\leavevmode
    \begin{enumerate}[label=\roman*)]
      ...
      \begin{align*}
        ...
        &= \frac{n!}{(n-k)!(n-n+k)!}\\
        &= \frac{n!}{(n-k)!(n-(n-k)!)}\\
        &= \binom{n}{n-k} && \text{Definición}
      \end{align*}
    \end{enumerate}
  \end{proof}
  
  \item \[\binom{n}{k} = \binom{n-1}{k-1}+\binom{n-1}{k} \qquad \text{(Formula recursiva del coeficiente binomial)}\]
  
  \item \[\binom{n+1}{k} = \binom{n}{k} + \binom{n}{k-1} \qquad \text{(Teorema de Pascal)}\]
  \begin{proof}\leavevmode
    \begin{align*}

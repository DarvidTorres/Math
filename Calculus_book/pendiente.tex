...
\begin{enumerate}[label=\alph*)]
  ...
  \begin{proof}\leavevmode
    Por definición, $a\leq \sup (A)$, y $\inf(B) \leq b$. Notemos que $a$ es una cota inferior de $B$, por lo que $a\leq \inf(B)\leq b$. Similarmente, $b$ es una cota superior de $A$, por lo que $a\leq \sup(A)\leq b$.
    
    Supongamos que $\inf(B) < \sup(A)$, entonces $\sup(A) - \inf(B) > 0$. Sea $\epsilon = \sup(A) - \inf(B)$, entonces $\exists b_0\in B$ tal que
    \begin{align*}
      \inf(B) &\leq b_0 < \inf(B) + \sup(A) - \inf(B)\\
      \inf(B) &\leq b_0 < \sup(A)
    \end{align*}
    Por lo que $b_0$ no es una cota superior de $A$, pero eso contradice la hipótesis de que $a\leq b$, $\forall a\in A$ y $\forall b\in B$
  \end{proof}

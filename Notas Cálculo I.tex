...
\subsection*{Lista de Ejercicios 2 (LE2)}
    ...
    \vspace{-1em}\begin{proof} 
        Por la propiedad del elemento identidad para la multiplicación tenemos que $1\cdot 1 = 1$. Además, el inverso multiplicativo de $1$ satisface que $1 \cdot 1^{-1}=1$. Debido a que el inverso multiplicativo de cada número real distinto de cero es único, de la igualdad anterior sigue que $1=1^{-1}$.
    \end{proof} \vspace{-1em}
...
\subsection*{Lista de Ejercicios 3 (LE3)}
    ...
    \item Sea $a$ un número real arbitrario pero fijo, demuestre que: $(-1) \cdot a =-a $. (Multiplicación por -1).
    ...
    \item Sean $a$ y $b$ números reales, demuestre que: $ (-a) \cdot (-b) = a \cdot b $.% (Ley de los signos: menos por menos es más).
...
\pagebreak
    ...
    Reutilizar \textit{esquemas} de proposiciones probadas nos permite agilizar la escritura de demostraciones; establecer notación tiene el mismo propósito. No obstante, cada vez que acordemos el uso de notación esta debe ser justificada; en ocasiones la justificación es tan simple como utilizar diferentes \textit{etiquetas} para los mismos \textit{objetos}, pero abusar de las etiquetas puede derivar en falta de rigor, como —por ejemplo, acordar que $m\cdot 0$ denotará el neutro aditivo $0$. Si bien, $m\cdot 0$ satisface las misas propiedades que $0$ (axiomas del neutro e inverso aditivo) se estaría prescindiendo de la demostración de la multiplicación por cero, con lo que se perdería información y clarificación del hecho de que $m\cdot 0 = 0, \forall m\in \R$. Con esto en mente, procedemos a establer algo de notación.
    ...
    Podemos usar esta notación sin ambigüedad ya que hemos demostrado que $(-a)\cdot b = -(a \cdot b) = a \cdot (-b)$.

    \item Si $m_1$, $m_2$ y $m_3$ son números reales, representaremos con el símbolo $m_1+m_2+ m_3$ a la suma $m_1+ \left(m_2+ m_3 \right)$.
    
    Ya que hemos verificado que todas las formas de sumar tres números reales son equivalentes, tenemos que $a+b+c$ representa la suma de cualesquiera números reales $a$, $b$ y $c$ sin importar la asociatividad presente.
...
\pagebreak
    ...
    \begin{itemize}
        \item[$\Rightarrow)$] \begin{align*}
            -a &< b && \text{Hipótesis}\\
            -a+a-b &< b +a-b && \text{Ley de la cancelación}\\
        ...
        \item[$\Leftarrow)$] \begin{align*}
            -b &< a && \text{Hipótesis}\\
            -b + b-a &< a + b-a && \text{Ley de la cancleación}\\
...
\pagebreak
    ...
    Sabemos que $b\neq 0$ y $d\neq 0$. También, por definición, $\frac{c}{d}-\frac{a}{b}\in \R^+$, es decir, $\frac{bc-ad}{bd}>0$. Como $b$ y $d$ son distintos de cero, tenemos que $bd\neq 0$. Asimismo, $bc-ad\neq 0$.
    ...
    \end{align*}
    Como $\frac{bc-ad}{bd}>0$, entonces $\frac{d}{b+d}>0$ y $\frac{b}{b+d}>0$. Por esto, $b+d\neq 0$. Finalmente, tenemos dos casos: \begin{enumerate}[label=\roman*)]
        \item Si $b>0$, entonces $b+d>0$ y $d>0$.
        \item Si $b<0$, entonces $b+d<0$ y $d<0$.
    \end{enumerate}

    Por tanto, debe cumplirse que $b$ y $d$ deben ser ambos positivos o negativos.
...
\vspace{-1em}\begin{proof} 
    ...
    \begin{enumerate}[label=\roman*)]
        ...
        \item Si $a+b<0$, entonces $|a+b|=-a-b$. Como, $-a \leq |a|$ y $-b \leq |b|$, entonces, $-a-b \leq |a|+|b|$. Por tanto, $|a+b| \leq |a|+|b|$.
    \end{enumerate}    
\end{proof} \vspace{-1em}

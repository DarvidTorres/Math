...
\begin{enumerate}[label=\alph*)]
 ...
 \item Como $n>0$, se tiene $n+1>1$, por definición \begin{align*}
 ...
 \item Sea $m>n$. Notemos que si $m=1$ tenemos que $m\leq n$, pero esto contradice nuestro supuesto, por lo que $m>1$, y por definición, $a^m=a\cdot a^{m-1}$. Luego, \begin{itemize}
  \item Si $n=1$, entonces $a^n=a$, en cuyo caso \begin{align*}
    \frac{a^m}{a^n} &= \frac{a\cdot a^{m-1}}{a}\\
    &= a^{m-1}\\
    &= a^{m-n}
  \end{align*}
  \item Si $n>1$, entonces $a^n=a\cdot a^{n-1}$, en cuyo caso \begin{align*}
    a^{m-n} &= a^{m-n+1-1} \\
    &= a^{m-1-n+1}\\
    &= a^{m-1-(n-1)}\\
    \frac{a^m}{a^n} &= \frac{a\cdot a^{m-1}}{a\cdot a^{n-1}}\\
    &= \frac{a^{m-1}}{a^{n-1}}
  \end{align*}
  Sabemos que $n-1 \in\N$, pues $n$ y $1$ son números naturales, y $n>1$. Además, como $a\neq 0$ por hipótesis, denotamos $\frac{1}{a^{n-1}}$ como $a^{-\bigl(n-1\bigr)} = a^{1-n}$, sin embargo, para tener que $frac{a^{m-1}}{a^{n-1}} = a^{(m-1)+(1-n)}$ requerimos que $1-n\in \N$, lo cual no siempre es el caso. Por lo tanto, requerimos expandir el conjunto de $\N$.
 \end{itemize}
 
 Notemos que $m-n\in \N$, y por definición \begin{align*}
  a^{m-n} = \left\{
   \begin{array}{@{}r@{\thinspace}l}
    a, &  \ \text{si}  \ m-n = 1\\
    a\cdot a^{m-n-1}, &  \ \text{si}  \ m-n> 1
   \end{array} \right.
  \end{align*}
 \end{enumerate}

 Como $n\in \N$, si $m=1$ tenemos que $m\leq n$, pero esto contradice nuestro supuesto inicial, por lo que $m>1$ y por definición, $a^m=a\cdot a^{m-1}$. Luego, $a^n=a$ si $n=1$ y $a^n=a\cdot a^n-1$ si $n>1$.
 \end{proof}

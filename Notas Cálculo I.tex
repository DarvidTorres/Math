...
\begin{enumerate}[label=\alph*)]
 ...
 \textbf{Proposición:} Si $m\neq n$, y $a\neq 0$, entonces $\frac{a^m}{a^n} = a^{m-n}$. El siguiente es un esbozo de la prueba propuesta por un estudiante:

Consideremos el caso cuando $m>n$. Notemos que si $m=1$ tenemos que $m\leq n$, pero esto contradice nuestro supuesto, por lo que $m>1$, y por definición, $a^m=a\cdot a^{m-1}$. Luego, \begin{itemize}
  \item Si $n=1$, entonces $a^n=a$, en cuyo caso \begin{align*}
    \frac{a^m}{a^n} &= \frac{a\cdot a^{m-1}}{a}\\
    &= a^{m-1}\\
    &= a^{m-n}
  \end{align*}
  \item Si $n>1$, entonces $a^n=a\cdot a^{n-1}$, en cuyo caso
  ...
  \begin{align*}
    ...
    %&= a^{m-1-(n-1)}\\
  \end{align*}
  Sabemos que $n-1 \in\N$, pues $n$ y $1$ son números naturales, y $n>1$. Además, como $a\neq 0$ por hipótesis, denotamos $\frac{1}{a^{n-1}}$ como $a^{-\bigl(n-1\bigr)} = a^{1-n}$, sin embargo, para tener que $\frac{a^{m-1}}{a^{n-1}} = a^{m-1}\cdot a^{1-n} = a^{(m-1)+(1-n)}$ requerimos que $1-n\in \N$, como se probó en (c) de LE8, pero dado que $n>1$, sigue que $0>n-1$ con $n-1\neq \N$, por lo que no podemos emplear dicha proposición. Por lo tanto, requerimos expandir el conjunto de $\N$.
 \end{itemize}

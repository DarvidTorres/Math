...
\textbf{\textit{Teorema.}} Todo conjunto finito tiene elemento mínimo y elemento máximo, es decir, para un conjunto finito $A$ no vacío, $\exists m,n\in A$ tales que $m\leq a\leq n, \forall a\in A$.

\textbf{Demostración:}

Sea $n\in \N$ y $A \coloneqq \{a_1, \dots, a_n\}$ no vacío.

Procedemos por inducción sobre el número de elementos de $A$. \begin{enumerate}[label=\roman*)]
    \item Si $n=1$, tenemos $A\coloneqq\{a_1\}$, por lo que $m=a_1$ y $n=a_1$ cumplen con la condición requerida.
    \item Supongamos que la propiedad se cumple para $n=k$.
    \item Definimos al conjunto $A\coloneqq \{a_1, \dots, a_k, a_{k+1}\}$. Por hipótesis de inducción, el conjunto \[A' \coloneqq A \setminus \{a_{k+1}\} = \{a_1, \dots, a_k\}\] tiene elemento mínimo y máximo, es decir, $\exists m',n'\in A'$ tales que $m'\leq a' \leq n', \forall a' \in A'$.
    
    Notemos que todo elemento de $A$ es miembro de $A'$ o es igual a $a_{k+1}$. Observemos ahora que $a_{k+1}$ cumple con alguno de los siguientes casos:
    \begin{enumerate}[label=\alph*)]
        \item Si $a_{k+1}<m'$, tenemos que $a_{k+1}<m'\leq a' \leq n'$, entonces elegimos $m=a_{k+1}$ y $n=n'$.
        \item Si $m' \leq a_{k+1}\leq n'$, entonces $a_{k+1}\in A'$ y $m'\leq a_{k+1} \leq n'$, por lo que $m=m'$ y $n=n'$.
        \item Si $a_{k+1}>n'$, tenemos que $m'\leq a' \leq n'<a_{k+1}$, entonces elegimos $m=m'$ y $n=a_{k+1}$.
    \end{enumerate}
    En cualquier caso $A$ tiene elemento mínimo y máximo. \qed
\end{enumerate}
...
\begin{enumerate}[label=\alph*)]
    \item Sea $X$ una secuencia convergente. Por definición, $\exists n_0 \in \N$ tal que los términos $x_{n\geq n_0}$ satisfacen que $|x_{n\geq n_0}-\ell|<\epsilon, \forall \epsilon>0$. Sea $\epsilon_0>0$ arbitrario pero fijo, entonces \begin{align*}
        |x_{n\geq n_0} - \ell| &< \epsilon_0 \\
        |x_{n\geq n_0} - \ell| + |\ell| &< \epsilon_0 + |\ell|
    \end{align*}
    Luego, notemos que $|x_{n\geq n_0}| = |x_{n\geq n_0}-\ell+\ell|$, y por la desigualdad del triángulo, \[|x_{n\geq n_0}-\ell+\ell| \leq |x_{n\geq n_0}-\ell| + |\ell|\] Por transitividad, $|x_{n\geq n_0}|< \epsilon_0 + |\ell|$, lo que implica que $\{x_{n\geq n_0}\}$ está cotado superiormente.
    
    Por otra parte, notemos que el conjunto de índices $n<n_0$ está acotado superiormente, y por esto, $\{x_{n<n_0}\}$ es finito y por tanto, tiene cota superior. %proof https://math.stackexchange.com/questions/548806/a-finite-set-always-has-a-maximum-and-a-minimum

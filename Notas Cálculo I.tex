...
Existe un conjunto llamado conjunto de los números reales, denotado por $\R$, cuyos elementos llamaremos números reales. Este conjunto está dotado con dos operaciones binarias: \begin{enumerate}[label=\roman*)]
    \item Suma: $+$
...
\begin{enumerate}[label=S\arabic*.]
    \item[S0.] Cerradura (de la suma).
    
    La suma es cerrada. Esto significa que para cualesquiera números reales $m$ y $n$ se verifica que $m+n \in \R$.
...
Si $a$, $b$ y $c$ son números reales tales que $a+c=b+c$, entonces $a=b$. El siguiente es un esbozo de la prueba propuesta por un estudiante:
%%Utilizando los axiomas enunciados hasta ahora \textbf{S[1-4]}, podemos obtener resultados útiles, por ejemplo, si tenemos números reales $a,b,c$ tales que $a+c=b+c$, encontraremos que $a=b$. El lector acostumbrado a matemáticas de bachillerato, podría intentar demostrar este hecho de la siguiente manera: 
\vspace{-1em}\begin{align*}
    ...
    a &= b
\end{align*}
Aunque en principio el resultado anterior no es incorrecto, debemos justificar cada \textit{paso} mediante las propiedades conocidas hasta este punto. Por lo anterior, una forma más precisa de demostrar la proposición es la siguiente:
...
\end{proof} \vspace{-1em}
    ...
    Sea $a\neq 0$. Supongamos que $-a=0$. Por este teorema $-a=-0$; por la unicidad del inverso aditivo, sigue que $-a$ es inverso aditivo de $0$, por lo que $a=0$, lo que contradice nuestro supuesto. Por tanto, $-a\neq 0$.
    %Otra forma:
    ...
    \end{enumerate}
\end{enumerate}
Continuamos enunciando los axiomas:
...
Notemos que el axioma de \textbf{asociatividad} (de la multiplicación) no incluye todas las formas en que podríamos multiplicar tres números reales (en general diferentes) $a$, $b$ y $c$:
...
Si $a,b\in \R$ son tales que $a+(-b)=b+(-a)$, entonces $a=b$. El siguiente es un esbozo de la prueba propuesta por un estudiante: \begin{align*}
    a+(-b) &=b+(-a) && \text{Hipótesis}\\
    a-b &= b-a && \text{Notación}\\
    (a-b) + b+ a &= (b-a) +b+a && \text{Ley de cancelación de la suma}\\
    a+b-b+a &= b+b+a-a && \text{Conmutatividad}\\
    a+0+a &= b+b+0 && \text{Inverso aditivo}\\
    2a&= 2b && \text{Notación}\\
    a &= b && \text{¿Ley de cancelación de la multiplicación?}
\end{align*}
%\begin{align*}
%a+(-b)&=b+(-a) && \text{Hipótesis} \\
...
El lector notará que la ley de la cancelación de la multiplicación exige que el número a \textit{ser cancelado} sea diferente de $0$. Sin embargo, hasta este punto no hemos demostrado que $2\neq 0$, pero los axiomas que hemos listado y los resultados que hemos derivado de ellos no son suficientes para probar este hecho (el lector debería verificarlo). Procedemos a añadir elementos a nuestro conjunto de axiomas:
...
\begin{enumerate}[label=O\arabic*)]
    \item Para cada número real $m$ se cumple una y sólo una de las siguientes condiciones (Tricotomía):
        ...
        \end{enumerate}
\end{enumerate}
\textbf{\textit{Definición:}}\begin{itemize}
...
\end{itemize}
%
\textbf{Nota:} Es imediato que $0\notin \R^+$ y $0\notin \R^-$, pues si suponemos que $0\in \R^+$ o $0\in \R^-$, tenemos que $0<0$, lo que contradice el axioma de tricotomía.
...
\textbf{\textit{Definición:}} Sean $m$ y $n$ números reales.\begin{itemize}
...
\end{itemize}
%
%\textbf{\textit{Definición:}} Sean $a$ y $b$ números reales, decimos que: \begin{itemize}
...
%    \end{proof} \vspace{-1em}
%
%    \textbf{\textit{Definición:}} Si $x$ es un número real tal que $0<x$ diremos que $x$ es un númeo real positivo.
...
%        \end{itemize}
%        \textbf{\textit{Definición:}} Si $x$ es un número real tal que $x<0$, diremos que $x$ es un número real negativo.
%        %\begin{center}\vspace{-1em}
        ...
        %\textbf{\textit{Definición:}} El conjunto de los números reales negativos se representa con el símbolo $\R^-$ y se define como los elementos en los números reales cuyos inversos aditivos pertenecen al conjunto de los números reales positivos:
    %\[
...
\pagebreak
    ...
    \vspace{-1em}\begin{proof} 
        Por definición $b-a >0$ y $c-b >0$. Por la cerradura de la suma, $(b-a) + (c-b) >0$. De donde sigue que $(b-a)+(c-b)=b-a+c-b=c-a>0$, es decir, $a<c$.
    \end{proof} \vspace{-1em}
        %\begin{align*}
        ...
        \end{proof} \vspace{-1em}
        \textbf{\textit{Definición:}} Al número $\frac{a+b}{2}$ lo llamaremos el punto medio entre $a$ y $b$.
...
\end{enumerate}
%\textbf{\textit{Definición:}} El conjunto de los números reales no negativos es el conjunto: \[
%        \R^+\cup \set{0}
%        \]

\section*{Valor absoluto}

\textbf{\textit{Definición:}} Sea $a$ un número real, definimos el valor absoluto de $a$, denotado por $|a|$ como sigue:
...
\textbf{\textit{Definición:}} Sea $A\subset \R$, decimos que $A$ es un conjunto inductivo si se cumplen las siguientes condiciones:
    \begin{enumerate}[label=\roman*)]
        \item $1 \in A$.
...
\textbf{\textit{Definición:}} Al conjunto $C$ de (3) de LE6 lo llamaremos conjunto de los números naturales y lo denotaremos con el símbolo $\N$.
...
\begin{enumerate}[label=\alph*)]
    ...
    \textbf{\textit{Corolario:}} \begin{enumerate}[label=\roman*)]
        ...
        Por definición, $x>0$, por lo que $x+n>n$. Por hipótesis, $x+n\in \N$ y $n\in \N$. Luego, por este teorema, $(x+n)-n \in \N$, osea, $x\in \N$.
    \end{enumerate} %Corolario
...
\textbf{\textit{Definición:}} Sea $A$ un subconjunto no vacío de $\R$, decimos que $A$ está acotado:
\begin{itemize}
...
\textbf{\textit{Definición:}} Sea $A\subset \R$ tal que $A$ es no vacío y está acotado superiormente, decimos que un número real $S$ es supremo de $A$ si $S$ satisface las siguientes condiciones:
\begin{itemize}
...
\textbf{\textit{Definición:}}. Sea $A$ un subconjunto no vacío del conjunto de los números reales, acotado inferiormente, decimos que un número real $L$ es ínfimo de $A$ si $L$ satisface las siguientes condiciones: \begin{itemize}
...
\textbf{\textit{Definición:}} Sean $a$ y $b$ objetos cualesquiera, definimos la pareja ordenada $(a,b)$ como sigue: \[
...
\textbf{\textit{Definición:}} Sea $a$, $b$ números reales, definimos el intervalo \begin{itemize}
...
\textbf{\textit{Definición:}} Una sucesión es una función %$X: n\in \N \mapsto x_n \in \R$
\begin{align*}
    X: \ & \N \to \R \\
    \ &  n \mapsto x_n 
\end{align*}
...
\textbf{\textit{Definición:}} Una sucesión $(x_n)$ es convergente si $\exists \ell \in \R$ tal que para cada $\varepsilon>0$ existe un número natural $n_\varepsilon$ (que depende de $\varepsilon$) de modo que los términos $x_n$ con $n\geq n_\varepsilon$ satisfacen que $|x_n-\ell|<\varepsilon$.
...
\textbf{\textit{Definición:}} Una sucesión es divergente si no es convergente.

\textbf{\textit{Definición:}} Una sucesión $(x_n)$ está acotada si $\exists M\in \R^+$ tal que $|x_n|\leq M, \forall n\in \N$.

...
\begin{enumerate}[label=\alph*)]
    \item Sean $\ell$ y $\ell'$ límites de la sucesión $(x_n)$. Sea $\varepsilon_0>0$ arbitraria pero fija, tenemos que $\exists n', n''$ tales que \begin{align*}
        |x_{n\geq n'}-\ell|<\varepsilon_0 && \text{(1)}\\
        |x_{n\geq n''}-\ell'|<\varepsilon_0 && \text{(2)}
    \end{align*}
    Sin pérdida de generalidad, sea $n'<n''$. Entonces, $x_{n''\geq n'}$ satisface —por (1)— que \[|\]
    \item Sea $(x_n)$ una sucesión convergente. Por definición, $\forall \epsilon>0, \exists n_\varepsilon \in \N$ tal que los términos $x_{n\geq n_\varepsilon}$ satisfacen que $|x_{n\geq n_\varepsilon}-x|<\epsilon$. Sean $\epsilon_0>0$ arbitrario pero fijo, entonces \begin{align*}
        |x_{n\geq n_0} - x| &< \epsilon_0 \\
        |x_{n\geq n_0} - x| + |x| &< \epsilon_0 + |x|
    \end{align*}
    Luego, notemos que $|x_{n\geq n_0}| = |x_{n\geq n_0}-x+x|$, y por la desigualdad del triángulo, \[|x_{n\geq n_0}-x+x| \leq |x_{n\geq n_0}-x| + |x|\] Por transitividad, $|x_{n\geq n_0}|< \epsilon_0 + |x|$, lo que implica que $\{x_{n\geq n_0}\}$ está cotado superiormente.
    
    Por otra parte, notemos que el conjunto de índices $n<n_0$ está acotado superiormente, y por esto, $\{x_{n<n_\varepsilon}\}$ es finito y por tanto, tiene cota superior. %proof https://math.stackexchange.com/questions/548806/a-finite-set-always-has-a-maximum-and-a-minimum
    
    Finalmente, el conjunto $\{x_{n<n_\varepsilon}\} \cup \{\epsilon_0 + |x|\}$ está acotado superiormente, por lo que $X$ está acotada. \qed
\end{enumerate}

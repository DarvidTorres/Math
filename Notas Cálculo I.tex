...
\newcommand{\qued}{\mbox{}\hfill $\square$}

\let\epsilon\varepsilon
\let\for\forall
%\let\empty\emptyset

% Macro for set definition
\def\defined #1{\coloneqq \left\{#1\right\}}
% e.g. $A \defined{n\in \N | n>1, n-1\in \N}$

%\def\for #1\in #2{, \forall #1 \in #2}
...
Si $A\subseteq \N$ y $A \neq \emptyset$, entonces $\exists c\in A$ tal que $c\leq a, \forall a\in A$.

\textbf{Demostración:}

Sea $A\subseteq \N$ con $A\neq \emptyset$. Supongamos que si $c\leq a, \forall a\in A$, entonces $c\notin A$.

Definimos el conjunto $L\defined{n\in \N: n\leq a, \forall a\in A}$. Si $1\notin L$, tendríamos que $\exists a_0\in A$ tal que $1>a_0$, pero $1\leq a_0$ —ya que $a_0\in \N$— lo cual es una contradicción, por tanto $1\in L$.

Si $n\in L$, tenemos que $n\leq a, \for a\in A$. Luego, si $n+1\notin L$, tendríamos que $\exists a_0\in A$ tal que $n+1>a_0$, por lo que $n\leq a_0<n+1$, y —por (h) de LE7— no puede ser el caso que $n<a_0$, de donde sigue que $n=a_0$, pero esto contradice nuestro supuesto inicial, entonces, debe ser el caso que $n+1\in L$. Consecuentemente, $L$ es un conjunto inductivo, y —por definición— $\N\subseteq L$ y $L\subseteq \N$, lo que implica que $L=\N$.

Como $A\subseteq L$ con $A\neq \emptyset$, $\exists a_0\in A$ tal que $a_0\leq a, \for a\in A$, lo cual es una contradicción. Por tanto, $\exists c\in A$ tal que $c\leq a, \forall a\in A$. \qed
...
\textbf{\textit{Axioma del supremo.}} Todo subconjunto no vacío del conjunto de los números reales que sea acotado superiormente tiene supremo.
...
\begin{enumerate}[label=\alph*)]
    ...
    \item Por la propiedad arquimediana, el conjunto $E\defined{m\in \N: y<m}$ es no vacío. Además, por el principio del buen orden, $E$ tiene elemento mínimo.
\end{enumerate}

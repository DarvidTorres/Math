...
\setlist[enumerate]{font=\bfseries} %bold all lists$
...
% \end{proof}
 ...
 \bfit{Corolario:}\begin{enumerate}[label=\Roman*)]
  ...
  \begin{enumerate}[label=\roman*)]
    ...
    \item $c<d\leq a <b$
  \end{enumerate}
  En particular, llamaermos a la \textit{forma} $a<b<c<d$, desigualdad \textit{interna}.
...
\begin{enumerate}[label=\alph*)]
  ...
  \begin{proof}\leavevmode
    Notemos que en la sumatoria $\sum_{m=s}^{t} \Bigl(f(n) \cdot  g(m)\Bigr)$, $f(n)$ es constante, por lo que
    \begin{align*}
